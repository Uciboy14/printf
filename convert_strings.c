...
/** * convert_s - Converts an argument to a
 * string and
 * stores it to a buffer contained in a struct.
 * @args: A va_list pointing to the
 * argument to be converted.
 * @flags: Flag modifiers.
 * @wid: A width modifier.
 * @prec: A precision modifier.
 * @len: A length modifier.
 * @output: A buffer_t struct containing a
 * character array.
 *
...
/**
 * convert_S - Converts an argument to a
 * string and stores it to a buffer
 ...
 *
 * Return: The number of bytes stored to the buffer.
 * Description: Non-printable
 * characteres (ASCII values < 32 or >= 127)
 * are stored as \x followed by the
 * ASCII code value in hex.
 */
unsigned int convert_S(va_list args, buffer_t *output, unsigned char flags, int wid, int prec, unsigned char len)
{
 ...
 * @args: A va_list pointing to the string
 * to be reversed. * @flags: Flag modifiers.
 * @wid: A width modifier.
 * @prec: A precision modifier.
 * @len: A length modifier.
...
unsigned int convert_R(va_list args, buffer_t *output, unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	for (i = 0; *(str + i) != '\0' && i < prec; i++)
	{
		...
		if (j == 52) ret += _memcpy(output, (str + i), 1);
	}
